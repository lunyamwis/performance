    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            if subtasks.is_valid():
                ...
                tasks_send = []
                get_members = []
                for member in subtasks.instance.subtask.all():
                    get_members.append(member.member_assigned.id)
                subs = subtasks.instance.subtask.filter(
                    member_assigned_id__in=get_members)
                site = self.request.build_absolute_uri(
                    '/task-master/my-tasks/')
                for task in range(len(subs)):
                    tasks_send.append(
                        f"{subs[task].name} assigned to {subs[task].member_assigned.username}")
                message = f'Timesheet Data Tables:'
                html_template = 'taskmanager/email.html'
                html_content = render_to_string(
                    html_template, {'taskset': subtasks.instance, 'site': site})
...
class TaskUpdate(BSModalUpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['update'] = True
        if self.request.POST:
            data['subtasks'] = SubTaskUpdateFormset(
                self.request.POST, instance=self.object)
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        assignment_typeset = Task.objects.get(
            id=request.POST.get('assignment_typeset'))
        ...
        for item in data:
            if item.startswith("subtask-") and item.endswith("-id") and data[item]:
                id_ = data[item]
                date_key = "subtask-{}-task_due_date".format(
                    item.split("-")[1])
                member_key = "subtask-{}-member_assigned".format(
                    item.split("-")[1])
...
def task_detail(request, task_id):
    ...
    if allowed:
        return render(
            ...
            {'user': request.user, 'task': task, 'subtasks': subtasks,
                'my_subtasks': my_subtasks, 'success_form': form}
        )
    else:
...
class ExtendDeadline(BSModalUpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['update'] = True
        if self.request.POST:
            data['subtasks'] = SubTaskExtendFormset(
                self.request.POST, instance=self.object)
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        for item in data:
            if item.startswith("subtask-") and item.endswith("-id") and data[item]:
                ...
                date_key = "subtask-{}-task_due_date".format(
                    item.split("-")[1])
                reason_key = "subtask-{}-extension_reason".format(
                    item.split("-")[1])
                task_due_date = data[date_key]
...
class Reassigning(BSModalUpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['update'] = True
        if self.request.POST:
            data['subtasks'] = SubTaskReassignFormset(
                self.request.POST, instance=self.object)
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        for item in data:
            if item.startswith("subtask-") and item.endswith("-id") and data[item]:
                ...
                member_key = "subtask-{}-member_assigned".format(
                    item.split("-")[1])
                reason_key = "subtask-{}-reassign_reason".format(
                    item.split("-")[1])
                member_assigned = data[member_key]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if task.assigned_to.get(username=request.user.username):
            if subtask.status == 'RET' or subtask.status == 'COMP':
                ...
                subtask.save()
                messages.success(
                    request, f"The task {subtask.name} has been returned for revision")
...
def subtask_delete(request, subtask_id):
    ...
    if subtask.task.assigned_to.get(username=request.user.username) and subtask.status == 'PLAN':
        subtask.delete()
        messages.success(
            request, 'You have successfully deleted the assignment. thank you')
        return redirect('taskmanager:task_my_tasks')
    else:
        raise PermissionDenied
...
def tasks(request):
    ...
    context['tasks'] = SubTask.objects.filter(
        Q(status="PLAN") & Q(member_assigned=request.user))
    context['completed_tasks'] = SubTask.objects.filter(
        Q(status="COMP") & Q(member_assigned=request.user))
    context['resubmitted_tasks'] = SubTask.objects.filter(
        Q(status="RET") & Q(member_assigned=request.user))
    context['revised_tasks'] = SubTask.objects.filter(
        Q(status="REV") & Q(member_assigned=request.user))
    context['approved_tasks'] = SubTask.objects.filter(
        Q(status="APP") & Q(member_assigned=request.user))
    context['invoiced_tasks'] = SubTask.objects.filter(
        Q(status='INV') & Q(member_assigned=request.user))
    context['pending_approval'] = SubTask.objects.filter(
        Q(status="PA") & Q(member_assigned=request.user))
    context['tasks_inprogress'] = SubTask.objects.filter(
        Q(status="PROG") & Q(member_assigned=request.user))
    ...
    context['completed_tasks'] = [*context['completed_tasks'], *
                                  context['resubmitted_tasks'], *context['pending_approval']]
    ...
    context['tasks_inprogress'] = [
        *context['tasks_inprogress'], *context['revised_tasks']]
    return render(
        request,
...
def team_tasks(request, team_id):
    ...
    context['completed_tasks'] = [
        *context['completed_tasks'], *context['resubmitted_tasks']]
    context['completed_or_approved_tasks'] = [*context['completed_tasks'],
                                              *context['approved_tasks'], *context['revised_tasks']]
    context['tasks_inprogress'] = [
        *context['tasks_inprogress'], *context['revised_tasks']]
    return render(
        request,
...
def task_accept(request, task_id):
    ...
    if request.user in task.team.members.all():
        if subtask.status == 'PLAN':
            ...
            task.save()
            messages.success(
                request, 'The task has been marked as accepted successfully. Thank you')
            return redirect('taskmanager:task_my_tasks')
        else:
            raise PermissionDenied
    else:
        if subtask.status == 'PLAN':
            ...
            subtask.save()
            messages.success(
                request, 'The task is waiting for approval for you to begin because this task is from an external team. thank you')
            return redirect('taskmanager:task_my_tasks')
        raise PermissionDenied
...
def task_resubmit(request, task_id):
    ...
    if request.user in task.team.members.all():
        if subtask.status == 'REV':
            ...
            subtask.save()
            messages.success(
                request, "The task has been successfully resubmitted. Thank you!")
            return redirect('taskmanager:task_my_tasks')
        else:
            raise PermissionDenied
    else:
    ...
    if request.user == task.creator or request.user in task.team.members.all():
        if subtask.status == 'PROG' or task.status == 'PROG':
            if task.task_count == 100:
                ...
                task.save()
                messages.success(
                    request, "The project has been successfully marked as completed. Thank you!")
            subtask.status = 'COMP'
            subtask.completed_date = datetime.date.today()
            subtask.save()
            site = request.build_absolute_uri('/task-master/my-tasks/')
            subject = f'Task Notification'
            message = f'The task {subtask.name} from the project {task.title} for the client {task.client_name} has been completed by {subtask.member_assigned}. And is ready for review please go to site {site}'
            target_email = [task.assigned_to.first().email]
            send_mail(subject, message, settings.FROM_EMAIL, target_email)
            messages.success(
                request, "The task has been successfully marked as completed. Thank you!")
            return redirect('taskmanager:task_my_tasks')
            ...
            subtask.save()
            messages.success(
                request, "The task has been successfully resubmited. Thank you!")
            return redirect('taskmanager:task_my_tasks')
        else:
            raise PermissionDenied
    else:
...
def task_mark_pending_approved(request, task_id):
    ...
    if request.user:
        if subtask.status == 'PA':
            ...
            subtask.save()
            messages.success(
                request, f"The assignment {subtask.name} has been approved and you can begin working on it. Thank you!")
            return redirect('taskmanager:task_my_tasks')
        else:
            raise PermissionDenied
    else:
...
def task_sources(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            form.save()
            messages.success(
                request, 'Project source has been successfully recorded')
